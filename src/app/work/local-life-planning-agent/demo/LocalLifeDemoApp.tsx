...
import { DEMO_DEFAULT_STATE, getDemoScenarioById, type DemoScenarioId } from "./data/demoScenarios";
...
export type LocalLifeDemoMode = "full" | "preview";

export type LocalLifeDemoAppProps = {
  mode: LocalLifeDemoMode;
};
...
export function LocalLifeDemoApp({ mode }: LocalLifeDemoAppProps) {
  const isPreview = mode === "preview";
  const rootHeight = isPreview ? "h-[520px]" : "min-h-[560px] h-full";
  ...
  const selectedPoi = useMemo(() => {
    ...
    return (
      mapPois.find((poi) => poi.id === selectedPoiId) ??
      result.rankedPois.find((poi) => poi.id === selectedPoiId)
    );
  ...
  return (
    <div
      className={`local-life-demo relative min-h-0 w-full overflow-hidden bg-[#f5f6f8] ${rootHeight}`}
      data-demo-mode={mode}
    >
